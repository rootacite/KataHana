...
变化图行、三张分析卡、六个显示开关（**Top moves 默认关**）、整局复盘按钮、候选点卡、
保存/另存/导出 SGF/复制 SGF（当前棋谱写入系统剪贴板）、
...
- **弹窗是屏幕布局内的 overlay，不是独立窗口**：`HanaScrimModal`（
  ...
  `FrostedSurface` 卡。卡片高度封顶为窗口减去边距，内容超出时内部滚动，避免窄横屏
  把 New Game / Seat 表单挤没。新建对局与座位配置打开 `slideFromBottom`：遮罩只做
