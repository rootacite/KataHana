...
This section describes how the engine is connected and how the small gateway that ships with the repository works. The engine settings screen contains the connection profile (a WebSocket URL, an optional token, and separate visit budgets for play and for review) together with actions to test the connection and to run a benchmark. The benchmark pings the engine with `query_version`, then measures overlay one-visit queries and search runs at 80, 400 and 2000 visits, plus a human-network probe. It reports network round-trip separately from a 0–100 hardware score (visits per second after subtracting RTT), so a fast GPU on a slow link is no longer scored the same as a weak CPU on localhost. It takes roughly 15 to 70 seconds.
