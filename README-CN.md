...
KataHana 是一个围棋客户端，用来做两件事：下棋，以及事后复盘。它用 Compose Multiplatform 编写，因此游戏逻辑与绝大部分界面都集中在同一个共享模块里，由桌面（JVM）版与 Android 版共用；仓库里还包含一个 iOS 应用壳。你的棋局归你自己所有：棋盘、规则、棋谱树与保存的对局都存放在你的设备上，没有需要注册的账号，也没有替你保管棋谱的云服务。但应用确实依赖一个引擎来“判断局面”——客户端在棋力与评估上刻意保持空白。AI 对手、胜率、目差曲线、势力图与整局复盘，全部来自一个它通过 WebSocket 与之对话的 KataGo 兼容进程。这个进程通常是仓库自带的网关，跑在桌面电脑上（手机在同一网络里把地址指过去即可）；但只要对方能说同一套协议，它可以是任何一台可达的机器。没有可达的引擎时，人人对弈、棋谱树与存档照常工作，但一切依赖判断的功能都会保持空白：客户端可用，只是明显功能残缺。

这个项目之所以存在，是因为围棋软件里有一个常见的两难取舍。许多工具都默认你具备两样东西之一：要么有一个在线服务可以下棋，要么本地已经装好了引擎。两种假设都有代价。服务器会把你的棋谱记录保存在别人的平台上，没有连接就用不了。引擎则通常需要针对某一种硬件打包与配置，这对 KataGo 来说尤其麻烦，因为它的二进制是跟随 GPU 分发的，无法做成一个通用的安装文件。KataHana 试图同时避开这两个假设：棋谱数据归你自己所有，引擎只是客户端按需连接的外部进程（通常是你自己机器或网络里的网关），仅在它可用时才与之通信。结果就是：人人对弈任何时候都能进行，而 AI 支持是加分项，而不是前提条件。
...
- **以树为基础的复盘模型。** 从最新一手往前 Undo，就进入了复盘：AI 在叶节点等待；若从某个中间节点落子，棋局就在那里分叉成一个新变化。Redo 沿首选变化前进，引擎从不进入非叶节点的局面行棋。在合法交叉点上长按或右键，可以把引擎的最好后续显示成虚拟棋子，而不会在树上长出新分支。
...
打开应用，首先看到的就是主屏：两张卡片浮在柔和的深色背景上。第一张卡片是最近对局列表。每条目显示对局的标题与一行摘要（棋盘尺寸、手数与对弈双方），整局存档就藏在这一行的背后。卡片上最多显示最近四盘，更多记录点 **More** 打开可滚动的浏览窗。因此，打开某一条记录，就会恢复到保存时的局面、棋谱树与形势曲线。候选着手与领地图这类实时数据并不存在存档里，所以会在你回到棋盘后向引擎重新请求。

<p align="center">
  <img src="screenshots/home.jpg" alt="主屏——最近对局、从 git 标签生成的更新日志，以及跟随最新标签的版本号"/>
</p>

第二张卡片是 **What's new**：一份由项目自身 git 历史生成的更新日志。打过标签的提交会带彩色徽章，没有一行是手写的——列表在构建时由 `git log` 生成，角落里的版本号取自最近的 git 标签（当前仓库为 `v1.7.1`；仓库没有任何标签时显示 `v0.1-alpha`）。这个机制在“代码结构”一节还会再提到，因为它实现在 Gradle 构建里，而不是应用代码里。
...
对局开始后，棋盘占据窗口中央，其余一切都围绕它排布。在窄屏/竖屏布局中，顶部细栏放置菜单、胜率条与 pass / undo / redo 操作——按住簇末的图标可临时预览 Top moves，不必打开开关；抽屉在横屏时从右侧、竖屏时从下侧滑出（点击粉色标签或滑动打开），容纳其余内容：轮到谁、提了多少子、候选点、棋谱树、形势图与质量表，以及表情、连络线、坐标、领地与死子等开关。在宽窗口与横屏手机上，棋谱树、形势图与质量表会改为常驻在棋盘旁的侧栏中，可拖外沿（三列时还可拖卡间缝）改宽度，对局过程中它们始终可见。
...
形势图与质量表里的评估是逐手收集的，这也是为什么这些功能只有在连接引擎之后才会出现。当前局面会显示带变化（PV）、损失目数与访问数的候选着手；**Analyze game** 会在后台沿整条首选变化逐节点请求完整分析。在复盘局面上，你还可以长按或右键一个合法交叉点，让引擎把双方的最好后续（最多 16 手）画成带手数的虚拟棋子；它们不会在棋谱树上长出新分支，领地热力会随着每一颗虚拟子落下而更新。

### 保存与记录

保存通过对局抽屉完成：**Save** 更新当前记录，**Save as** 以新名字另存一份。两者都会把对局连同逐手评估写入最近列表，这正是重启之后形势曲线与质量统计依然存在的原因。由于应用会跟踪保存之后局面是否发生变化，带着未保存的着手退出对局画面时，会弹出对话框，让你选择保存、放弃或取消。此外也可以打开与导出 SGF 文件，格式为 FF[4]——这也是绝大多数围棋工具使用的交换格式。**Copy SGF** 把当前棋谱直接写入系统剪贴板，不必经过文件对话框。
...
本节说明引擎如何连接，以及仓库附带的小型网关如何工作。引擎设置页包含连接配置（一个或多个用分号分隔的 WebSocket 地址、可选 token，以及 play 与 review 两套独立的访问量预算），以及“测试连接”和“运行基准测试”两个操作。填了多个地址时，客户端按顺序尝试，使用第一个握手成功的。基准测试会先用 `query_version` 测网络往返，再测单访问 overlay 查询、80 / 400 / 2000 访问量的搜索档位与一次人类网络探测。结论把网络延迟和硬件分数分开：硬件是减去 RTT 之后的 visits/s，映射到一条 0–100 的连续水平条，因此“远端强 GPU”不会再和“本机弱 CPU”落进同一档。大约耗时 15–70 秒。
...
由于引擎客户端期望诸如 `ws://127.0.0.1:2080` 这样的地址，仓库内置了一个恰好提供该地址的小型 Python 网关。这个地址只是默认值：网关可以跑在你控制的任何一台机器上，应用的引擎设置页也能指向别的地址——例如手机端把地址填成桌面电脑的局域网 IP。在项目根目录执行：
