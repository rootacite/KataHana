...
第二张卡片是 **What's new**：一份由项目自身 git 历史生成的更新日志。打过标签的提交会带彩色徽章，没有一行是手写的——列表在构建时由 `git log` 生成，角落里的版本号取自最近的 git 标签（当前仓库为 `v1.6.3`；仓库没有任何标签时显示 `v0.1-alpha`）。这个机制在“代码结构”一节还会再提到，因为它实现在 Gradle 构建里，而不是应用代码里。
...
本节说明引擎如何连接，以及仓库附带的小型网关如何工作。引擎设置页包含连接配置（WebSocket 地址、可选 token，以及 play 与 review 两套独立的访问量预算），以及“测试连接”和“运行基准测试”两个操作。基准测试会先用 `query_version` 测网络往返，再测单访问 overlay 查询、80 / 400 / 2000 访问量的搜索档位与一次人类网络探测。结论把网络延迟和硬件分数分开：硬件是减去 RTT 之后的 visits/s，映射到一条 0–100 的连续水平条，因此“远端强 GPU”不会再和“本机弱 CPU”落进同一档。大约耗时 15–70 秒。
